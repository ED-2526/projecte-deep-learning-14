...
CONFIG = {
    ...
    # Posa aquí el millor threshold que surti a evaluate_thresholds.py
    "threshold": 0.3,

    "out_dir": "results/ranked_predictions",
    "num_each": 4,
}
...
def save_prediction_figure(image, mask, pred, prob, dice, save_path, title):
    ...
    # Mostrem FLAIR com a imatge base.
    # En modalitats ["flair", "t1", "t1ce", "t2"], FLAIR és el canal 0.
    flair = image[0]
    ...
    for group_name, group_results in groups.items():
        ...
        for i, item in enumerate(group_results, start=1):
            save_path = os.path.join(
                group_dir,
                f"{group_name}_{i:02d}_idx_{item['idx']}_dice_{item['dice']:.4f}.png"
            )
